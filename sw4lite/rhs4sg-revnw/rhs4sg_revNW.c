...
// int klast, 		 int nk, int*  onesided, float*  a_acof, float * a_bope,
//		 float*  a_ghcof, float*  a_lu, float*  a_u,
//		 float*  a_mu, float*  a_lambda,
//		 float h, float*  a_strx, float*  a_stry,
//		 float* a_strz  )
// restrict qualifier does not seem to help much
// void rhs4sg_rev( int ifirst, int ilast, int jfirst, int jlast, int kfirst,
// int klast, 		 int nk, int*  onesided, float*  a_acof, float * a_bope,
// float*
// a_ghcof, float*  a_lu, float*  a_u, 		 float*  a_mu, float*  a_lambda,
// float h, float*  a_strx, float*  a_stry, 		 float* a_strz  )
void rhs4sg_rev(int ifirst, int ilast, int jfirst, int jlast, int kfirst,
...
#define ghcof(i) a_ghcof[i - 1]
  ...
  const float i12 = 1.0 / 12;
  const float i144 = 1.0 / 144;
  const float tf = 0.75;
  ...
  int k1, k2, kb;
  int i, j, k, q, m, qb, mb;
  float mux1, mux2, mux3, mux4, muy1, muy2, muy3, muy4, muz1, muz2, muz3, muz4;
  float r1, r2, r3, mucof, mu1zz, mu2zz, mu3zz;
  float lap2mu, u3zip2, u3zip1, u3zim1, u3zim2, lau3zx, mu3xz, u3zjp2, u3zjp1,
      u3zjm1, u3zjm2;
  float lau3zy, mu3yz, mu1zx, mu2zy, u1zip2, u1zip1, u1zim1, u1zim2;
  float u2zjp2, u2zjp1, u2zjm1, u2zjm2, lau1xz, lau2yz;

  const float cof = 1.0 / (h * h);
  ...
  if (onesided[5] == 1)
    k2 = nk - 6;
//    cout << "k1=" << k1 << " k2=" << k2 << " jfirst=" << jfirst << "
//    jlast="<<jlast<<" ifirst="<<ifirst<<" ilast="<<ilast<< endl;
...
#pragma omp parallel private(                                                  \
        k, i, j, mux1, mux2, mux3, mux4, muy1, muy2, muy3, muy4, r1, r2, r3,   \
            mucof, mu1zz, mu2zz, mu3zz, lap2mu, q, u3zip2, u3zip1, u3zim1,     \
            u3zim2, lau3zx, mu3xz, u3zjp2, u3zjp1, u3zjm1, u3zjm2, lau3zy,     \
            mu3yz, mu1zx, u1zip2, u1zip1, u1zim1, u1zim2, u2zjp2, u2zjp1,      \
            u2zjm1, u2zjm2, mu2zy, lau1xz, lau2yz, kb, qb, mb, muz1, muz2,     \
            muz3, muz4)
  {
#pragma omp for
    for (k = k1; k <= k2; k++)
...
#pragma ivdep
        for (i = ifirst + 2; i <= ilast - 2; i++) {
          ...
          /* 75 ops */
          lu(1, i, j, k) =
              i6 * (strx(i) * ((2 * mux1 + la(i - 1, j, k) * strx(i - 1) -
                                tf * (la(i, j, k) * strx(i) +
                                      la(i - 2, j, k) * strx(i - 2))) *
          ...
          /* 75 ops */
          lu(2, i, j, k) =
              i6 * (strx(i) * (mux1 * (u(2, i - 2, j, k) - u(2, i, j, k)) +
          ...
          /* 75 ops */
          lu(3, i, j, k) =
              i6 * (strx(i) * (mux1 * (u(3, i - 2, j, k) - u(3, i, j, k)) +
...
static inline void fill_constant(float *ptr, size_t n, float v) {
...
int main() {
  // Small grid
  int ifirst = 1, ilast = 10;
  int jfirst = 1, jlast = 10;
  int kfirst = 1, klast = 10;
  int nk = 12; // allow ghost indexing up to nk+1
  float h = 1.0f;

  const int ni = ilast - ifirst + 1;
  const int nj = jlast - jfirst + 1;
  const int nkspan = klast - kfirst + 1;
  const int nijk = ni * nj * nkspan;

  int *onesided = (int *)malloc(6 * sizeof(int));
  memset(onesided, 0, 6 * sizeof(int));

  // Coeff sizes (layout from macros in kernel): acof: 6*8*8 = 384, bope:
  // 6*8=48, ghcof: 8
  float *acof = (float *)malloc(6 * 8 * 8 * sizeof(float));
  memset(acof, 0, 6 * 8 * 8 * sizeof(float));
  float *bope = (float *)malloc(6 * 8 * sizeof(float));
  memset(bope, 0, 6 * 8 * sizeof(float));
  float *ghcof = (float *)malloc(8 * sizeof(float));
  memset(ghcof, 0, 8 * sizeof(float));

  // Populate simple SBP-like central weights for bope first column to avoid
  // total zeros This is only to exercise the paths without caring about
  // physical correctness
  if (bope != NULL) {
    bope[0] = -1.0f / 12.0f; // i=1, j=1
    bope[1] = 8.0f / 12.0f;
    bope[2] = 0.0f;
    bope[3] = -8.0f / 12.0f;
    bope[4] = 1.0f / 12.0f;
  ...
  float *strz = (float *)malloc((nk + kfirst) * sizeof(float));
  fill_constant(strx, ni + ifirst, 1.0f);
  fill_constant(stry, nj + jfirst, 1.0f);
  fill_constant(strz, nk + kfirst, 1.0f);

  rhs4sg_rev(ifirst, ilast, jfirst, jlast, kfirst, klast, nk, onesided, acof,
             bope, ghcof, lu, u, mu, lambda, h, strx, stry, strz);
